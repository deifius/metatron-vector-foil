...
Current multiplayer status: UI/audio/remote-trace scaffolding, a lightweight Flask room/invite coordination layer, identity-checked WebRTC signal lock, live `pilot_trace` telemetry, and a first host-authored `world_snapshot` channel now exist. Accepted callsigns can negotiate a P2P DataChannel, exchange heartbeats, see each other’s sphenic corsairs, and receive host scope contacts for enemy/sphere/team-state summaries. The host snapshots are rendered as authoritative scope contacts but do not yet mutate client physics, bullets, scoring, or shared game-over state. Combat scoring, full host-authoritative simulation, relay fallback, and ranked verification are still future passes. ICE servers are empty by default for privacy/LAN testing; configure `MVF_MULTIPLAYER_ICE_SERVERS_JSON` for remote STUN/TURN routing.
