...
This document describes the intended multiplayer addition for Metatron Vector FOIL. The current implementation is now an early unranked P2P shared-defense prototype: oscilloscope-style lobby/front-screen components, audio hook constants, remote pilot rendering, HUD/title cleanup, ephemeral Flask room/invite/status/signaling coordination, WebRTC DataChannel signal lock, live pilot traces, client-to-host pilot input, host-authored world snapshots, initial host-owned allied fire/combat authority, and host-side contribution/debrief accounting. It is playable as a first shared Sol-defense slice, but it is not ranked, server-verified, relay-hardened, host-migration-safe, or persistent across server restarts.
